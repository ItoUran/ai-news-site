/**
 * 気象庁(JMA)予報区分(都道府県・地方単位)の一覧。
 * https://www.jma.go.jp/bosai/common/const/area.json の offices セクションから生成(変更頻度が低いため静的化)。
 * 予報データ取得には https://www.jma.go.jp/bosai/forecast/data/forecast/{code}.json の code をそのまま使う。
 */
...
export const DEFAULT_AREA_CODE = "130000"; // 東京都

export function findJmaArea(code: string): JmaArea | undefined {
  return jmaAreas.find((a) => a.code === code);
}
