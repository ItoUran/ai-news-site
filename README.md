...
- **気象予報**: 気象庁(JMA)公式データをリアルタイム表示。今日〜明日の6時間刻みの
  降水確率タイムラインつき
...
- **未ログインでも閲覧可能**: 全タブは未ログインでも閲覧できますが、👍/👎・ブックマーク・
  パーソナライズにはログインが必要です(未ログイン時はログイン画面へ誘導)。
- **ブックマーク**: ログインユーザー限定。記事カードの👎ボタンの右にあるアイコンボタンで
...
- **タイトル**: 「9月9日 夕方のニュース」のように、月日+時間帯(朝/昼/夕方)で統一しています。
  こちらも同ファイルの `buildEpisodeTitle` による決め打ちで、AIには生成させていません
  (以前はAIに自由生成させていたため「AIニュースラジオ 2026年9月4日」「2026年9月4日の
  ニュース」のように回ごとに表記がバラバラだった)。
